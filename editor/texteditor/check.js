		...
		return {
			...
			on: function(b, d, g, k, h) {
				...
				if (0 > e.getListenerIndex(d)) {
					...
					p.fn = d;
					p.priority = h;
					for (var w = e.length - 1; 0 <= w; w--) if (e[w].priority <= h) return e.splice(w + 1, 0, p), {
						removeListener: q
					};
			...
			copyAttributes: function(a, b) {
				...
				for (var e = 0; e < c.length; e++) {
					var d = c[e],
						...
						f;
					if (!(w in b)) if ("checked" == w && (f = this.getAttribute(w))) a.setAttribute(w, f);
	...
	(function() {
		...
		function d(a, b, c, d, f) {
			...
			if (n && C.type == CKEDITOR.NODE_TEXT && (r.equals(C) || r.type === CKEDITOR.NODE_ELEMENT && r.getFirst().equals(C))) c.append(a.document.createText(C.substring(L, D)));
				...
				for (var O = r.getParents(), l = C.getParents(), K = m(), B = O.length - 1, A = l.length - 1, J = c, G, X, V, da = -1, Q = K; Q <= B; Q++) {
					X = O[Q];
					V = X.getNext();
				...
				J = c;
				for (Q = K; Q <= A; Q++) if (c = l[Q], V = c.getPrevious(), c.equals(O[Q])) b && (J = J.getChild(0));
				else {
		...
		CKEDITOR.dom.range.prototype = {
				...
				switch (a) {
					...
					case CKEDITOR.ENLARGE_ELEMENT:
						...
						for (n = c(n); n || r;) {
							...
							for (; r;) if (D = !1, r.type == CKEDITOR.NODE_COMMENT) r = r.getPrevious();
							else {
								if (r.type == CKEDITOR.NODE_TEXT) N = r.getText(), d.test(N) && (r = null), D = /[\s\ufeff]$/.test(N);
								else if ((r.$.offsetWidth > (CKEDITOR.env.webkit ? 1 : 0) || b && r.is("br")) && !r.data("cke-bookmark")) if (L && CKEDITOR.dtd.$removeEmpty[r.getName()]) {
									N =
									r.getText();
									
									if (d.test(N)) r = null;
									else
									for (var t = r.$.getElementsByTagName("*"), E = 0, y; y = t[E++];) if (!CKEDITOR.dtd.$removeEmpty[y.nodeName.toLowerCase()]) {
										r = null;
										break
									}
									r && (D = !! N.length)
							...
							for (; r;) {
								...
								if (r.type == CKEDITOR.NODE_TEXT) N = r.getText(), g(r, 0) || (r = null), D = /^[\s\ufeff]/.test(N);
									...
									if ((0 < r.$.offsetWidth || b && r.is("br")) && !r.data("cke-bookmark")) if (L && CKEDITOR.dtd.$removeEmpty[r.getName()]) {
										...
										for (t = r.$.getElementsByTagName("*"), E = 0; y = t[E++];) if (!CKEDITOR.dtd.$removeEmpty[y.nodeName.toLowerCase()]) {
											r = null;
											break
										}
										r && (D = !! N.length)
			...
			_find: function(a, b) {
				var c = this.getCommonAncestor(),
					...
					f = [],
					g, m, h, k;
				if (c && c.find) for (m = c.find(a), g = 0; g < m.count(); g++) if (c = m.getItem(g), b || !c.isReadOnly()) h = c.getPosition(d.startNode) & CKEDITOR.POSITION_FOLLOWING || d.startNode.equals(c), k = c.getPosition(d.endNode) & CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IS_CONTAINED || d.endNode.equals(c), h && k && f.push(c);
				return f
			}
		};
	...
	(function() {
		function a(a, b, c, d, e) {
			...
			for (d = a.length; c < d; ++c) {
				...
				for (I in l = f.elements, delete f.elements, l) if (b[I]) b[I][n ? "unshift" : "push"](f);
				else b[I] = [f]
			}
		}
		function d(a, c, d, e) {
		...
		function n(a, b, c) {
			...
			else if (t.$block[d] || "tr" == d) if ("br" == b) a.previous && !H(a.previous) && (b = u(), b.insertBefore(a)), a.next && !H(a.next) && (b = u(), b.insertAfter(a)), a.replaceWithChildren();
			else {
				var d = a.children,
					e;
				b: {
			...
			applyTo: function(a, b, c, d) {
				...
				a.forEach(function(a) {
					if (a.type == CKEDITOR.NODE_ELEMENT) {
						...
						if (!b || "span" != a.name || !~CKEDITOR.tools.objectKeys(a.attributes).join("|").indexOf("data-cke-")) if (g = z(e, a, f, E), g & 1) r = !0;
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load: function(a, d, b) {
			...
			for (var c = this.loaded, f = this.registered, g = [], k = {}, h = {}, p = 0; p < a.length; p++) {
				var q = a[p];
				if (q) if (c[q] || f[q]) h[q] = this.get(q);
				else {
			...
			F.onTagOpen = function(f, e, g, k) {
				...
				if (a(e)) m.push(e);
					...
					if ("br" == f) M.push(e);
						...
						for (; !(k = (g = u.name) ? CKEDITOR.dtd[g] || (u._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c, e.isUnknown || u.isUnknown || k[f]);) if (u.isOptionalClose) F.onTagClose(g);
		...
		a.prototype = {
			...
			exec: function(a, c) {
				var d = c instanceof CKEDITOR.htmlParser.node || c instanceof CKEDITOR.htmlParser.fragment,
					...
					h = k.length,
					p, q, e, x;
				for (x = 0; x < h; x++) if (d && (p = c.type, q = c.name), e = k[x], !(a.nonEditable && !e.options.applyToAll || a.nestedEditable && e.options.excludeNestedEditable)) {
	...
	(function() {
		...
		CKEDITOR.htmlParser.element.prototype = CKEDITOR.tools.extend(new CKEDITOR.htmlParser.node, {
			...
			filter: function(a, b) {
				...
				var p, q;
				for (p in k) {
					for (h = k[p];;) if (q = a.onAttributeName(b, p)) if (q != p) delete k[p], p = q;
					else
					break;
					else {
						delete k[p];
					...
					if (!this.is("textarea")) {
						...
						CKEDITOR.env.webkit && this.attachListener(a, "key", function(b) {
							...
							if (c in t && (b = a.getSelection(), 0 !== b.getRanges().length)) {
								...
								l = b.block,
								d = c.endPath().block,
								l && d && !l.equals(d) ? (a.fire("saveSnapshot"), (e = l.getBogus()) && e.remove(), c.enlarge(CKEDITOR.ENLARGE_INLINE), c.deleteContents(), d.getParent() && (d.moveChildren(l, !1), b.lastElement.mergeSiblings(), q(l, d, !0)), c = a.getSelection().getRanges()[0], c.collapse(1), c.optimize(), "" === c.startContainer.getHtml() && c.startContainer.appendBogus(), c.select(), b = !0) : b = !1;
								if (!b) return;a.getSelection().scrollIntoView();a.fire("saveSnapshot");
								return !1
							}
						}, this, null, 100)
					}
			...
			getRanges: function() {
				...
				function() {
					...
					var b = function(b, c) {
						...
						for (var e = d.children, f, g, h = b.duplicate(), k = 0, m = e.length - 1, n = -1, t, q; k <= m;) if (n = Math.floor((k + m) / 2), f = e[n], h.moveToElementText(f), t = h.compareEndPoints("StartToStart", b), 0 < t) m = n - 1;
						else if (0 > t) k = n + 1;
						else
						return {
							container: d,
			...
			if (c.collapsed) e = M(this, e), c.insertNode(e), c.moveToPosition(e, CKEDITOR.POSITION_BEFORE_END);
				...
				for (r.getPosition(w) == CKEDITOR.POSITION_FOLLOWING && (r = 0); r;) {
					...
					if (r.equals(w)) r = null, m = !0;
						...
						if (D) if (E = r.getParent(), D = h, H = g, I = k, !E || !(E.getDtd() || CKEDITOR.dtd.span)[H] && !I || D.parentRule && !D.parentRule(E)) m = !0;
						else {
...
*/
		...
		CKEDITOR.plugins.copyformatting = {
			...
			_getSelectedWordOffset: function(a) {
				...
				function e(a, c) {
					...
					(f && f.getStyle && ("none" == f.getStyle("display") || !f.getText()));
			...
			var v = CKEDITOR.tools.addFunction(k),
				w = CKEDITOR.tools.addFunction(function(b, e) {
					...
					switch (e.getKeystroke()) {
						case d ? 39:
							37: case 9:
							return (d = CKEDITOR.document.getById(g + (b + 1))) || (d = CKEDITOR.document.getById(g + "0")),
							d.focus(),
							!1;
						case d ? 37:
							39: case CKEDITOR.SHIFT + 9:
							return (d = CKEDITOR.document.getById(g + (b - 1))) || (d = CKEDITOR.document.getById(g + (h.list.length - 1))),
							d.focus(),
							!1;
						case 27:
							return a.focus(),
	...
	(function() {
		...
		function m(a) {
			...
			for (var f = new CKEDITOR.dom.walker(b), d; d = f.next();) if (d.type == CKEDITOR.NODE_ELEMENT) if (!d.equals(a.data.node) && d.getDirection()) b.setStartAfter(d), f = new CKEDITOR.dom.walker(b);
			else {
	...
	(function() {
		...
		CKEDITOR.plugins.link = {
			getSelectedLink: function(c, b) {
				...
				for (d = 0; d < f.length; d++) if (g = a.getRanges()[d], g.shrink(CKEDITOR.SHRINK_ELEMENT, !0, {
					skipBogus: !0
			...
			parseLinkAttributes: function(c, b) {
				...
				if (!g.type) if (f = a.match(x)) g.type = "anchor", g.anchor = {}, g.anchor.name = g.anchor.id = f[1];
	...
	(function() {
		...
		function u(d, f) {
			for (var b = D(d) ? d : q(d), c = b[0].getAscendant("table"), e = w(b, 1), b = w(b), a = f ? e : b, g = CKEDITOR.tools.buildTableMap(c), c = [], e = [], b = [], k = g.length, h = 0; h < k; h++) c.push(g[h][a]), e.push(f ? g[h][a - 1] : g[h][a + 1]);
		...
		function z(d) {
			if (d instanceof CKEDITOR.dom.selection) {
				...
				for (d = b.length - 1; 0 <= d; d--) z(b[d]);e ? p(e, !0) : c && (f[0].moveToPosition(c, CKEDITOR.POSITION_BEFORE_START), f[0].select(), c.remove())
	...
	(function() {
		...
		function z(a, b) {
			...
			l > p && (c = l, l = p, p = c);
			for (c = g; c <= h; c++) for (g = l; g <= p; g++) d = new CKEDITOR.dom.element(e[c][g]), d.$ && !d.getCustomData("selected_cell") && (k.push(d), CKEDITOR.dom.element.setMarker(f, d, "selected_cell", !0));
			...
			v = {
				...
				upload: function() {
					var c = {
						...
						_beginUpload: function(a, b) {
							...
							if (!1 !== a.fire("uploadStarted", b) && a.progressReporterType) if (!a._isLoaderDone(b)) a.wrapper.addClass("cke_widget_wrapper_uploading"), g = new a.progressReporterType, a.wrapper.append(g.wrapper), g.bindLoader(b);
				...
				link: function() {
					...
					return {
						...
						setUp: function(d) {
							d.plugins.link && (d.contextMenu && c(d), d.on("dialogShow", function(c) {
								var b = p(d),
									e = c.data,
									h, k;
						...
						data: function(a) {
							...
							if ("undefined" !== typeof c) if (null === c) this.parts.link.remove(!0), this.parts.link = null, delete a.data.link;
							else {
								a = this.parts;
		...
		CKEDITOR.ui.balloonPanel.prototype = {
			...
			attach: function() {
				...
				return function(p, l) {
					...
					t && this.editor.elementMode === CKEDITOR.ELEMENT_MODE_INLINE && (m = this._getViewPaneRect(u), m.right += this.triangleWidth, m.bottom += this.triangleHeight);
					q ? (CKEDITOR.tools.array.forEach(q, function(a) {
						this._adjustElementRect(a, t ? m : r)
					}, this), g = this._getAlignments(q[0], d, e), 1 < q.length && (g["bottom hcenter"] = this._getAlignments(q[1], d, e)["bottom hcenter"]), h = {
						"top hcenter": !0,
						"bottom hcenter": !0
		...
		CKEDITOR.plugins.add("cloudservices", {
			...
			beforeInit: function(a) {
				...
				a.on("fileUploadRequest", function(b) {
					...
					a instanceof CKEDITOR.plugins.cloudservices.cloudServicesLoader && (d.file = d.upload, delete d.upload, c ? b.data.fileLoader.xhr.setRequestHeader("Authorization", c) : (CKEDITOR.error("cloudservices-no-token"), b.cancel()))
				}, null, null, 6);
				a.on("fileUploadResponse", function(a) {
	...
	(function() {
		...
		CKEDITOR.plugins.add("easyimage", {
			...
			init: function(a) {
				if (!CKEDITOR.env.ie || 11 <= CKEDITOR.env.version) m || (CKEDITOR.document.appendStyleSheet(this.path + "styles/easyimage.css"), m = !0), a.addContentsCss && a.addContentsCss(this.path + "styles/easyimage.css")
			},
